  ...
  const submitFloor = () => {
    if ($floorStates !== undefined) {
      ...
      if(current?.blueprint){
        $floorStates = $floorStates.set($activeFloor, { ...current, height: floor.height, innerWallVectors: [], outerWallCorners: getPoints(), outerWallWidth: floor.outerWallWidth });
      }
    }
...
<Stage bind:handle={stage} config={{ width, height }}>
  <FloorPlanner bind:circles bind:isClosed blueprint={blueprint} />
</Stage>
  ...
  <span transition:fade={{duration: 150}}>
      ...
      <Card.Footer class="flex justify-between">
        ...
        <Dialog.Root>
          ...
          >Continue</Dialog.Trigger
          >
          <Dialog.Content class="sm:max-w-[425px]">
            <Dialog.Header>
              <Dialog.Title>Add a floor</Dialog.Title>
              <Dialog.Description>
                Add some details about the floor!
              </Dialog.Description>
            </Dialog.Header>
